...
func QueryFakeUsers(i int) {
	...
	if err == nil {
		for index := 0; index < i+1; index++ {
			...
			defer rdb.Close()
			id, _ := rdb.Get(redis.Ctx, "user_count").Result()
			rdb.Do(redis.Ctx, "incr", "user_count")
			rdb.Do(redis.Ctx, "ZADD", "username", id, username)
			rdb.Do(redis.Ctx, "RPUSH", "firstname", firstname)
			rdb.Do(redis.Ctx, "RPUSH", "lastname", lastname)
			rdb.Do(redis.Ctx, "ZADD", "email", id, email)
			rdb.Do(redis.Ctx, "RPUSH", "regdate", regdate)
		}
	}
