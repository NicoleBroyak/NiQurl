...
func RandomUser() string {
	...
	un, _ := RDB.ZRange(Ctx, "username", n, n).Result()
	return un[0]
}
...
func insertUser(i int, u *UsersStruct) {
	...
	RDB.ZAdd(Ctx, "username", &redis.Z{
		Score:  float64(id),
		Member: u.Results[i].Login.Username,
	})
	RDB.RPush(Ctx, "firstname", u.Results[i].Name.First)
	RDB.RPush(Ctx, "lastname", u.Results[i].Name.Last)
	RDB.ZAdd(Ctx, "email", &redis.Z{
		Score:  float64(id),
		Member: u.Results[i].Email,
	})
	RDB.RPush(Ctx, "regdate", u.Results[i].Registered.Date)
}
